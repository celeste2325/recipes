...
public interface RecipeRepository extends JpaRepository<Recipe, Integer> {
    ...
    @Query(value = "SELECT r FROM Recipe r inner join User usu on r.userID = usu.userID inner join IngredientUsed u on r.recipeID = u.recipeID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE u.ingredientID <> ?1 and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2 )) order by usu.name asc")
    List<Recipe> recetasSinIngredientesOrdenadaPorNombreUser(Integer idIngrediente, Integer idUsuario);
    @Query(value = "SELECT r FROM Recipe r inner join User usu on r.userID = usu.userID inner join IngredientUsed u on r.recipeID = u.recipeID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE u.ingredientID <> ?1 and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2 )) order by r.name asc")
    List<Recipe> recetasSinIngredientesOrdenadaPorNombre(Integer idIngrediente, Integer idUsuario);
    @Query(value = "SELECT r FROM Recipe r inner join User u on r.userID = u.userID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE r.name like ?1% and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2)) order by r.name")
    List<Recipe> findByNombreLikeIgnoreCase(String s, Integer idUsuario);

    @Query(value = "SELECT r FROM Recipe r inner join User u on r.userID = u.userID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE r.name = ?1 and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2)) order by u.name")
    ...
    @Query(value = "SELECT r FROM Recipe r inner join DateOfRecipe f on r.recipeID = f.recipeID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE r.name = ?1 and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2)) order by f.dateCreation desc")
    ...
    @Query(value = "SELECT r FROM Recipe r inner join User u on r.userID = u.userID inner join AuthorizationStatus es on es.entityID = r.recipeID WHERE r.recipeID = ?1 and ((es.statusType='Autorizado' and es.entityType='Receta') OR (es.statusType='No autorizado' and es.entityType='Receta' and r.userID = ?2)) order by r.name")
